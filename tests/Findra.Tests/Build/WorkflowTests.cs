...
public class WorkflowTests
{
    ...
    private static string[] RunBlocks(string yaml) =>
        [.. RunBlocksWithShell(yaml).Select(b => b.Body)];

    /// <summary>Every <c>run: |</c> block, with the shell it runs under: the step's own
    /// <c>shell:</c> if it names one, otherwise the default of the runner its job is on - bash on
    /// ubuntu and macOS, PowerShell on Windows.</summary>
    private static (string Shell, string Body)[] RunBlocksWithShell(string yaml)
    {
        string[] lines = yaml.Replace("\r\n", "\n", StringComparison.Ordinal).Split('\n');
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            string shell = ShellAt(lines, i, depth);
            var body = new List<string>();
            ...
            blocks.Add((shell, string.Join("\n", body.Select(l => l.Length >= indent ? l[indent..] : l))));
        }

        return [.. blocks];
    }

    /// <summary>The shell the step holding line <paramref name="at"/> runs under.</summary>
    private static string ShellAt(string[] lines, int at, int depth)
    {
        int start = at;
        while (start > 0 && !Regex.IsMatch(lines[start], $@"^\s{{0,{Math.Max(0, depth - 1)}}}- ")) start--;

        for (int j = start; j < lines.Length; j++)
        {
            if (j > at && Regex.IsMatch(lines[j], $@"^\s{{0,{Math.Max(0, depth - 1)}}}\S")) break;
            Match named = Regex.Match(lines[j], @"^\s*shell:\s*(?<shell>\S+)");
            if (named.Success) return named.Groups["shell"].Value;
        }

        for (int j = at; j >= 0; j--)
        {
            Match runner = Regex.Match(lines[j], @"^\s*runs-on:\s*(?<os>\S+)");
            if (runner.Success)
                return runner.Groups["os"].Value.StartsWith("windows", StringComparison.Ordinal) ? "pwsh" : "bash";
        }
    ...
    public void EveryPowerShellBlockInEveryWorkflowParses()
    {
        ...
        foreach (string path in AllWorkflows().Concat(Directory.GetFiles(Repo.Path_("build"), "*.ps1")))
        {
            ...
            // A block that runs under bash is not PowerShell, and parsing it as PowerShell fails on
            // the first `$(...)` for reasons that say nothing about whether it works. Those are
            // counted above and left out here; the single-line runs in them are all plain commands.
            HashSet<string> bash = script
                ? []
                : [.. RunBlocksWithShell(text).Where(b => b.Shell == "bash").Select(b => b.Body)];

            foreach (string block in blocks.Where(b => !bash.Contains(b)).Concat(lines))
            {
    ...
    /// <summary>
    /// The edge function's own test runs on every push.
    ///
    /// <para>The site's Markdown negotiation is TypeScript running on Deno at Netlify's edge, and
    /// the xUnit suite cannot execute it. Its table of Accept headers lives in a node file beside
    /// it and is run here - which is the only reason node appears in any workflow. If this step is
    /// deleted, the one decision that can hand raw Markdown to every human visitor is checked by
    /// nothing at all.</para>
    ///
    /// <para>Node is still not a build prerequisite, and this test asserts that too: it belongs to
    /// this workflow and must not appear in the release or catalogue ones, which build and ship
    /// artefacts from committed files.</para>
    /// </summary>
    [Fact]
    public void TheMarkdownNegotiationIsCheckedOnEveryPush()
    {
        ...
        // are generated by hand and what they produced is committed.
        Assert.DoesNotContain("setup-node", Release, StringComparison.Ordinal);
        Assert.DoesNotContain("setup-node", Winget, StringComparison.Ordinal);
    }

    /// <summary>
    /// CI regenerates the site and requires that nothing changes.
    ///
    /// <para>The written pages are produced by <c>build/Make-Pages.mjs</c>, run by hand. The
    /// sentence-level tests catch prose that drifts from its source; this catches everything else
    /// the generator writes - a footer, a title, a version stamp - on the day somebody edits a
    /// source and forgets to run it.</para>
    /// </summary>
    [Fact]
    public void TheSiteIsRegeneratedOnEveryPushAndMustNotChange()
    {
        Assert.Contains("node build/Make-Pages.mjs", Ci, StringComparison.Ordinal);
        Assert.Contains("git diff --exit-code -- website/public", Ci, StringComparison.Ordinal);
    }

    /// <summary>
    /// The search engines are told after a release, and only after one, and only once it is live.
    ///
    /// <para>A release is the one push certain to change what the pages say, so it is the one
    /// submission that is true; a submission on every push trains an engine to discount the signal.
    /// And a submission sent before the deploy lands points the engines at pages that still say the
    /// old thing. So the workflow starts from a finished release run, never from a push, and waits
    /// for the live page to name the new version before it sends anything.</para>
    /// </summary>
    [Fact]
    public void TheSearchEnginesAreToldAfterAReleaseIsLive()
    {
        string site = Repo.Read(".github/workflows/site.yml");
        string triggers = site.Split("jobs:")[0];

        Assert.Contains("workflow_run:", triggers, StringComparison.Ordinal);
        Assert.Contains("workflows: [release]", triggers, StringComparison.Ordinal);
        ...
        Assert.Contains("conclusion == 'success'", site, StringComparison.Ordinal);

        int wait = site.IndexOf("data-stamp", StringComparison.Ordinal);
        int send = site.IndexOf("node build/Ping-IndexNow.mjs --send", StringComparison.Ordinal);
        Assert.True(wait >= 0 && send > wait, "the submission is not preceded by a wait for the live version");
    }
