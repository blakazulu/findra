...
Six diagnostic modes, built from day one, non-negotiable:
...
| `--searchbench [out.md]` | measure it, reproducibly, and print numbers fit to publish |
...
`--searchbench` exists because the README makes performance claims and every one of them
must be a measurement. It runs a fixed query set many times against the live index and
emits, as a Markdown fragment ready to paste:

- cold-start MFT enumeration time and the resulting name count, per volume
- name-query latency over the query set - median, p95, worst - separating the pipe round
  trip from the index scan
- resident bytes of the name index, and the on-disk size of each store
- content indexing throughput in files/minute, by kind
- the machine it ran on: CPU, RAM, disk class, Windows build

Numbers without the machine beside them are marketing, not measurement, so the machine line
is not optional.

## 9a. The README

The README is a product page, not a developer note. It has to make someone who has never
heard of Findra want it, which means it carries screenshots and numbers - and both must be
real.

**Screenshots are real renders.** `--searchshot` draws the actual card with the actual
painter, so every image in the README is the product rather than a mockup of it. A
screenshot is regenerated by running a command, never hand-edited, and the command that
produced each one is recorded next to it in the repo so anyone can reproduce it. The set
covers the resting capsule, a live result card, the advanced popup, the first-run download
screen and the settings window, in at least one dark and one light palette.

**Every number is measured, attributed, and reproducible.** Performance claims come from
`--searchbench` output pasted verbatim, with the machine named. Model sizes come from the
real files on disk, not the declared floors. No claim appears in the README that a reader
cannot reproduce with a command from the README itself.

**No comparative claims against named competitors**, because Findra cannot benchmark them
fairly and a claim it cannot defend is worse than no claim.
